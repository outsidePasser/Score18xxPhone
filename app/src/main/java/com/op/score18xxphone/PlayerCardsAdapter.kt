...
class PlayerCardsAdapter : RecyclerView.Adapter<PlayerCardsAdapter.PlayerCardViewHolder>() {
    ...
    class PlayerCardViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(playerIndex: Int) {
            ...
            data class ShareEntry(val label: String, val chipColor: Int, val textColor: Int, val count: Int)

            val entries = game.companies.mapNotNull { company ->
                val shareCount = company.shares.getOrElse(playerIndex) { 0 }
                if (shareCount > 0) ShareEntry(company.name, company.colorInt(), company.textColorInt(), shareCount) else null
            }.toMutableList()

            if (player.otherAssets != 0) {
                entries.add(ShareEntry("Other", "#808080".toColorInt(), "#FFFFFF".toColorInt(), player.otherAssets))
            }
            ...
            entries.chunked(2).forEach { pair ->
                ...
                pair.forEach { entry ->
                    ...
                    companyNameView.text = entry.label
                    companyNameView.background.colorFilter =
                        BlendModeColorFilterCompat.createBlendModeColorFilterCompat(
                            entry.chipColor, BlendModeCompat.SRC_ATOP
                        )
                    companyNameView.setTextColor(entry.textColor)
                    shareView.findViewById<TextView>(R.id.player_share_count).text = entry.count.toString()
